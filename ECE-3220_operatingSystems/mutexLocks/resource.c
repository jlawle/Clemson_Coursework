/* CPSC/ECE 3220 resource allocation code
  ...
  lawler6 - John Lawler - C10372381 
  February 22 2020
  Program 2 Resource Allocation using a Mutex lock and condition variable
...
resource_t * resource_init( int type, int total ){
    ...
    rc = pthread_cond_init( &r->condVar, NULL);
...
void resource_reclaim( resource_t *r ){
    ...
    // ADD call to pthread_cond_destroy() for each condition variable
     pthread_cond_destroy( &r->condVar);
